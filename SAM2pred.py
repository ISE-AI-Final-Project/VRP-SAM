...
from vrp_segment_anything.segment_anything import sam_model_registry
...
class SAM_pred(nn.Module):
    ...
    def forward_img_encoder(self, query_img):
        query_img = F.interpolate(
            query_img, (1024, 1024), mode="bilinear", align_corners=True
        )
    ...
    def get_prompt(self, protos, points_mask=None):
        if points_mask is not None:
            ...
            postivate_pos = (
                point_mask.squeeze(0).nonzero().unsqueeze(0) + 0.5
            ) * 64 - 0.5
            postivate_pos = postivate_pos[:, :, [1, 0]]
            point_label = torch.ones(postivate_pos.shape[0], postivate_pos.shape[1]).to(
                postivate_pos.device
            )
    ...
    def forward_prompt_encoder(self, points=None, boxes=None, protos=None, masks=None):
        q_sparse_em, q_dense_em = self.sam_model.prompt_encoder(
            points=points, boxes=None, protos=protos, masks=None
        )
        return q_sparse_em, q_dense_em

    def forward_mask_decoder(
        self, query_feats, q_sparse_em, q_dense_em, ori_size=(512, 512)
    ):
        ...
        low_masks = F.interpolate(
            low_res_masks, size=ori_size, mode="bilinear", align_corners=True
        )
    ...
    def forward(self, query_img, query_name, protos, points_mask=None, save_feat=False):
        ...
        protos, point_prompt = self.get_prompt(protos, points_mask)
        with torch.no_grad():
            # -------------save_sam_img_feat-------------------------
        ...
        low_masks, binary_mask = self.forward_mask_decoder(
            query_feats, q_sparse_em, q_dense_em, ori_size=(h, w)
        )
