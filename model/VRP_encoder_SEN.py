...
from common.utils import (
    get_mask_by_input_strokes,
    get_random_points_from_mask,
    get_stroke_preset,
)
from model.DETR.criterion import SetCriterion
from model.DETR.matcher import build_matcher
...
def get_point_mask(mask, training, max_points=20):
    ...
    if training:
        num_points = random.Random().randint(
            1, max_points
...
def get_scribble_mask(
    ...
    """
    if training:
        stroke_preset_name = random.Random().choices(
            stroke_preset, weights=stroke_prob, k=1
        )[0]
        nStroke = random.Random().randint(1, min(20, mask.sum().item()))
    else:
        stroke_preset_name = random.Random(321).choices(
            stroke_preset, weights=stroke_prob, k=1
        )[0]
...
class VRP_encoder_SEN(nn.Module):
    def __init__(self, args, backbone, use_original_imgsize, aux_loss=True):
        super(VRP_encoder_SEN, self).__init__()
        ...
        self.merge_support = nn.Sequential(
            nn.Conv2d(
                (hidden_dim * 2 + 1) * n_shot,
                hidden_dim,
                kernel_size=1,
        ...
        self.transformer_decoder = transformer_decoder(
            args, args.num_query, hidden_dim, hidden_dim * 2, num_layers=1
        )
        ...
        # SEN: DETR Head
        self.class_embed = nn.Linear(hidden_dim, 2)  # is-object [id:0] and not-object
    ...
    def forward(self, condition, query_img, support_img, support_mask, training):
        ...
        if condition == "scribble":
            support_mask_ori = get_scribble_mask(
                support_mask, training
            )  # scribble_mask
        ...
        supp_feat_1 = self.merge_1(
            torch.cat([supp_feat, supp_feat_bin, support_mask * 10], 1)
        )
        query_feat_1 = self.merge_1(
            torch.cat([query_feat, supp_feat_bin, pseudo_mask * 10], 1)
        )

        protos = self.transformer_decoder(query_feat_1, supp_feat_1, support_mask=None)

        # print(protos.shape)

        outputs_class = self.class_embed(protos)
        outputs_coord = self.bbox_embed(protos).sigmoid()
    ...
    def mask_feature(self, features, support_mask):
        ...
        feat_h, feat_w = supp_feat.shape[-2:][0], supp_feat.shape[-2:][1]
        area = (
            F.avg_pool2d(mask, (supp_feat.size()[2], supp_feat.size()[3]))
            * feat_h
            * feat_w
            + 0.0005
        )
    ...
    def forward_nshot(
        self, condition, query_img, support_imgs, support_masks, training, nshot=2
    ):
        """
        Forward n support imgs,

        Args:
            query_img : 1 x 3 x H x W
            support_imgs : 1 x n_shot x 3 x H x W
            support_masks : 1 x n_shot x H x W
        """
        assert support_imgs.shape[1] == nshot
        ...
        for n in range(nshot):
            support_img = support_imgs[:, n]
            support_mask = support_masks[:, n]
            ...
            with torch.no_grad():
                ...
                pseudo_mask = self.get_pseudo_mask(
                    supp_feat_4, query_feat_4, support_mask
                )

            supp_feat = self.downsample_query(supp_feat)

            prototype = self.mask_feature(supp_feat, support_mask)
            supp_feat_bin = prototype.repeat(
                1, 1, query_feat.shape[2], query_feat.shape[3]
            )

            # Add to list
            query_feat_all.extend([query_feat, supp_feat_bin, pseudo_mask * 10])
            support_feat_all.extend([supp_feat, supp_feat_bin, support_mask * 10])

        # Merge
        query_feat_1 = self.merge_support(torch.cat(query_feat_all, 1))
        supp_feat_1 = self.merge_1(torch.cat(support_feat_all, 1))

        # print("Query Feat shape", query_feat_1.shape)
        # print("Supp Feat shape", supp_feat_1.shape)
    ...
    def predict_mask_nshot(self, args, batch, sam_model, nshot, input_point=None):
        ...
        low_masks, pred_mask = sam_model(
            batch["query_img"], batch["query_name"], protos, input_point
        )
    ...
    def get_pseudo_mask(self, tmp_supp_feat, query_feat_4, mask):
        """
        compute a pseudo-mask for the query feature map based on the similarity between the query and support feature maps.
        """

        resize_size = tmp_supp_feat.size(2)
        tmp_mask = F.interpolate(
            mask, size=(resize_size, resize_size), mode="bilinear", align_corners=True
        )
...
def build_SEN(args, device):
    """
    Build VRP_encoder and criterion
    """

    model = VRP_encoder_SEN(
        args, args.backbone, use_original_imgsize=False, aux_loss=args.aux_loss
    )
