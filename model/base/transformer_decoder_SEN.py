...
class transformer_decoder_SEN(nn.Module):
    ...
    def __init__(
        ...
        # SEN : Add Bounding box regression head (outputs [x, y, w, h])
        self.bbox_head = MLP(hidden_dim, hidden_dim, 4, num_layers=3)

        # SEN : Add Confidence score head (outputs confidence)
        self.confidence_head = nn.Sequential(
            nn.Linear(hidden_dim, 1), nn.Sigmoid()  # Output between 0 and 1
        )
    ...
    def forward(self, x, x_s, support_mask):
        ...
        # SEN : Bounding box predictions
        bbox_predictions = self.bbox_head(output)  # Shape: [num_queries, bs, 4]

        # Confidence score predictions
        confidence_scores = self.confidence_head(output).squeeze(
            -1
        )  # Shape: [num_queries, bs]

        # Reshape for final output
        bbox_predictions = bbox_predictions.permute(
            1, 0, 2
        )  # Shape: [bs, num_queries, 4]
        confidence_scores = confidence_scores.permute(1, 0)  # Shape: [bs, num_queries]

        # Combine outputs into a single tensor
        predictions = torch.cat(
            [bbox_predictions, confidence_scores.unsqueeze(-1)], dim=-1
